...
if [ -f "$SCRIPT_DIR/proxmox_config.sh" ]; then
    . "$SCRIPT_DIR/proxmox_config.sh"
else
    echo "Error: proxmox_config.sh not found in $SCRIPT_DIR. Please create it first."
    exit 1
fi

# File to track if discovery was already sent
PUBLISHED_FILE="/tmp/proxmox_mqtt.published"

# File to track when SMART was already checked
SMART_CACHE="/tmp/proxmox_smart.lastrun"

# Paths for caching stats to avoid 'sleep'
CPU_CACHE="/tmp/proxmox_cpu.cache"
NET_CACHE="/tmp/proxmox_net.cache"
...
mqtt_publish() {
    ...
    local device_id="proxmox_${node_name}_${hw_addr}"
    ...
    local state_topic="proxmox/${node_name}_${hw_addr}/${group_id}/${entity_id}/state"
    ...
    if [[ ! -f "$PUBLISHED_FILE" ]] || ! grep -q "$unique_id" "$PUBLISHED_FILE" 2>/dev/null; then
        ...
        json_data+="${options},"
        json_data+="\"dev\":{\"ids\":[\"${device_id}\"],\"name\":\"${node_name}\",\"mdl\":\"${model_name}\",\"mf\":\"${manufacturer}\",\"sn\":\"${hw_addr}\",\"sw\":\"${pve_version}\"}"
        json_data+="}"
